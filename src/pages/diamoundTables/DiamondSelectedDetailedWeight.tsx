...
///
export const DiamondSelectedDetailedWeight = ({ subTableData }: any) => {
  /////////// VARIABLES
  ///
  ...
  useEffect(() => {
    if (queryClient) {
      const allQueries = selectedRow?.map((item: any) => {
        const Categories_ids = item.weightitems.map(
          (item: any) => item.category_id
        )
        const categoriesWeights = item.weightitems.map(
          (item: any) => item.weight
        )
        const finaleItem = {
          categories:
            allCategories &&
            Categories_ids.map((categoryDetailedId: any, index: number) => {
              return {
                categoriesname: allCategories
                  .filter((category: any) => categoryDetailedId == category.id)
                  .flat()[0]?.name,
      ...
      <ul className="grid grid-cols-3">
        {queryData?.flat()?.map((item: any) => (
          <div
            ...
            className="col-span-1  p-2 rounded flex gap-5 mx-2 my-2"
          >
            <li className="list-disc">
              <strong>{t("name")}</strong>: {item.categoriesname} ({t("weight")}
              :{item.categoriesWeight})
            </li>
