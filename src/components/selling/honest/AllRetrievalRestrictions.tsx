...
import { OneRetrievalRestrictions } from "./OneRetrievalRestrictions"
...
///
export const AllRetrievalRestrictions = () => {
    ///
    /////////// CUSTOM HOOKS
    ///
    ...
    const { data: honestBondsData, isLoading: honestBondsLoading, refetch, isRefetching } = useFetch({
        queryKey: ['all-honest-bonds-returned'],
        endpoint:  (search === `branchSafety/api/v1/return-bonds/${userData?.branch_id}?` || search ==='')
        ? `branchSafety/api/v1/return-bonds/${userData?.branch_id}?page=${page}` : `${search}`,
        pagination: true
    })
    /////////// VARIABLES
    ///
    ...
    const getSearchResults = async (req: any) => {
        let uri = `branchSafety/api/v1/return-bonds/${userData?.branch_id}?`;
        let first = false;
            ...
            <Button
                action={() => {
                    if (!Object.keys(selectedItem).length) {
                        ...
                        navigate(`/selling/honesty/all-retrieval-restrictions/${selectedItem.id!}`)
                    }
                }}
            >{t('next')}</Button>
        </div> */}
        <Modal isOpen={restrictModal} onClose={() => setOpenRestrictModal(false)} >
            <OneRetrievalRestrictions sanadId={selectedItem.id}/>
        </Modal>
    </div>
}